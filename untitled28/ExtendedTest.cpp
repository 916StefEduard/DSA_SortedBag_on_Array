...
bool increasing(TComp comp1,TComp comp2){
    if(comp1<=comp2){
        return true;
    }
    return false;
...
void testIteratorSteps(SortedBag& bag) {
	int count = 0;
	SortedBagIterator bi = bag.iterator();
	while (bi.valid()) {
		count++;
		bi.next();
	}
...
void testIterator() { 
	...
	SortedBag b2(increasing);
	for (int i = -100; i < 100; i++) { 
		b2.add(i);
		b2.add(i);
		b2.add(i);
	}
	SortedBagIterator it3 = b2.iterator();
	assert(it3.valid() == true); 
	for (int i = 0; i < 600; i++) {
		TComp e1 = it3.getCurrent();
		it3.next();
	}
	assert(it3.valid() == false);
	it3.first();
	assert(it3.valid() == true);
	SortedBag b3(increasing);
	for (int i = 0; i < 200; i = i + 4) { 
		b3.add(i);
	}
	SortedBagIterator it4 = b3.iterator();
	assert(it4.valid() == true);
	int count = 0;
	while (it4.valid()) { 
		TComp e = it4.getCurrent();
		assert(e % 4 == 0);
		it4.next();
		count++;
	}
	assert(count == 50);
	SortedBag b4(increasing);
	for (int i = 0; i < 100; i++) {
		b4.add(i);
		b4.add(i * (-2));
		b4.add(i * 2);
	...
	vector<TComp> elements;
	SortedBagIterator it5 = b4.iterator();
	while (it5.valid()) {
		TComp e = it5.getCurrent();
	...
	for (unsigned int i = 0; i < elements.size(); i++) { 
		TComp lastElem = elements.at(elements.size() - i - 1);
		assert(b4.search(lastElem) == true);
		b4.remove(lastElem);
	}

	SortedBag b5(increasing);
	for (int i = 0; i < 100; i++) {
		b5.add(i);
		b5.add(i * (-2));
		b5.add(i * 2);
	...
	vector<TComp> elements2;
	SortedBagIterator it6 = b5.iterator();
	while (it6.valid()) {
		TComp e = it6.getCurrent();
	...
	for (unsigned int i = 0; i < elements2.size(); i++) { 
		TComp firstElem = elements2.at(i);
